...
예를 들어 `renderWithHooks` 함수에서 현재 렌더링하려는 컴포넌트의 파이버 노드를 나타내는 `current` 가 존재하지 않았다면 마운트 단계이니 `mountState` 함수가 호출되고, `current.memoizedState` 가 존재한다면 업데이트 단계이니 `updateState` 함수가 호출되는 것입니다.
...
이제 이어서 컴포넌트가 업데이트될 때 호출되는 `updateState` 함수를 살펴보기에 앞서 훅 객체를 업데이트하는 [updateWorkInProgressHook](https://github.com/facebook/react/blob/v18.2.0/packages/react-reconciler/src/ReactFiberHooks.new.js#L657) 함수를 살펴보았습니다.

> 💡 **전제 사항**  
> React는 새롭게 렌더링하려고 하는 작업물을 임시의 `workInProgress` 라는 곳에서 처리하다가, 작업이 완료되면 `current` 에 반영하는 방식으로 동작하는 더블 버퍼링 방식을 사용합니다. 왜냐하면 새로운 작업을 처리하다가도 우선순위가 더 높은 작업이 있다면 기존의 작업을 중단하는 것에 용이하기 때문입니다.

> 그래서 리액트 소스코드 내부에서 전반적으로 사용되는 프로퍼티의 의미를 염두하고 있다면, 코드를 이해하기에 더 수월할 것입니다.  
> **current**: 이미 기존에 렌더링되어 있는 Fiber 노드 or Hook 객체 등을 의미합니다.  
> **workInProgress**: 현재 새롭게 렌더링하려고 하는 Fiber 노드 or Hook 객체 등을 의미합니다.  
...
function updateWorkInProgressHook(): Hook {
  ...
  if (nextWorkInProgressHook !== null) {
    // 다음으로 작업해야 할 nextWorkInProgressHook 훅이 존재한다면, 새로운 훅을 만들지 않고 그 훅을 재사용
    workInProgressHook = nextWorkInProgressHook
    nextWorkInProgressHook = workInProgressHook.next

    currentHook = nextCurrentHook
  } else {
    // 다음으로 작업해야 할 nextWorkInProgressHook 훅이 존재하지 않는다면, 새로운 훅을 만들어서 연결 리스트의 끝에 추가
...
- **7줄**: 후행하는 로직에 따라 current 파이버 노드의 훅 객체가 `nextCurrentHook` 프로퍼티에 할당됩니다.
- **10줄**: `currentlyRenderingFiber` 는 현재 처리하고 있는 컴포넌트의 파이버 노드인 `workInProgress` 파이버 노드를 가리키고 있는데, 이것의 `alternate` 는 이미 렌더링되었던 `current` 파이버 노드를 가리키고 있습니다. 즉, 이전에 이미 렌더링되어 있는 `current` 파이버 노드의 훅 객체를 참조합니다.
- **18줄**: `updateWorkInProgressHook` 함수가 종료되기 전에 이미 처리했었던 훅 객체를 `currentHook` 전역 프로퍼티에 할당하는데, 덕분에 두 번째 이상으로 선언된 훅을 처리할 때에는 복잡한 비교 로직 없이 바로 `next` 프로퍼티를 참조하여 다음 훅 객체를 처리할 수 있습니다.

#### nextWorkInProgressHook

현재 처리하고 있는 컴포넌트의 `workInProgress` 파이버 노드에 들어있는 훅 객체를 참조하는 식별자입니다.

- **21줄**: 후행하는 로직에 따라 workInProgress 파이버 노드의 훅 객체가 `nextWorkInProgressHook` 프로퍼티에 할당됩니다.
- **24줄**: 첫 번째로 선언된 훅을 처리할 때에는 `workInProgress` 파이버 노드의 훅 객체를 참조합니다.
- **27줄**: 두 번째 이상의 훅을 처리할 때에는 이미 처리했던 훅 객체인 `workInProgressHook` 의 `next` 포인터를 활용하여 다음 훅 객체를 참조합니다.
...
- **35줄, 43줄**: 다음으로 처리해야 할 훅 객체를 처리할 때 이전 훅 정보를 사용할 수 있도록 하기 위해서 `updateWorkInProgressHook` 함수가 종료되기 전에 이미 처리했던 `nextCurrentHook` 훅 객체를 `currentHook` 전역 프로퍼티에 할당합니다.

#### workInProgressHook

마찬가지로 이전에 `updateWorkInProgressHook` 가 종료되었을 때의 `nextWorkInProgressHook` 값을 참조하는 식별자입니다.

- **57줄, 60줄**: 마찬가지로 다음으로 처리해야 할 훅 객체를 위해서 이미 처리했던 `nextWorkInProgressHook` 객체를 `workInProgressHook` 전역 프로퍼티에 할당합니다. 또한 훅 객체를 연결합니다.
...
[updateReducer](https://github.com/facebook/react/blob/v18.2.0/packages/react-reconciler/src/ReactFiberHooks.new.js#L759) 는 컴포넌트가 업데이트되는 단계에 `useState` 나 `useReducer` 훅의 선언이 있을 때 호출되는 함수입니다.

Render 단계에서 `useState` 의 반환 값 중 두 번째로 넘어오는 setter 함수인 `dispatchSetState` 를 호출하게 되면 상태의 업데이트를 즉시 처리하는 것이 아니라 Queue에 넣어두고 있는데, 이 Queue에 들어있는 업데이트를 처리하는 과정이 `updateReducer` 에 포함되어 있기도 합니다.
...
이 부분도 Lane이나 Concurrent Rendering과 관련된 내용은 아직 이해가 부족하여 상태 업데이트에 대한 내용만 살펴보았습니다.

- **8줄**: 상태를 업데이트 할 동작을 나타내는 `Update` 객체를 생성합니다.
- **17줄**: 현재 Render 단계를 진행중이라면, 상태를 즉시 업데이트하는 것이 아니라 큐에 넣어두는 작업을 하는 것으로 보입니다. [enqueueRenderPhaseUpdate](https://github.com/facebook/react/blob/v18.2.0/packages/react-reconciler/src/ReactFiberHooks.new.js#L2318) 함수의 내용을 살펴보면 Update 객체를 원형 연결 리스트의 형태로 만들고 있음을 확인할 수 있습니다.
...
React 소스코드를 살펴보면서 Hook이 어떻게 클로저로 구현되어 있는지, 왜 Hook을 최상단에서만 선언해야 하는지 등에 대한 의문점을 해소할 수 있었습니다.
